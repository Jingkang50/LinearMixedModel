...
class LMM:
   ...
   def __init__(self,Y,K,Kva=[],Kve=[],X0=None,verbose=False):
      ...
      Kva and Kve can be computed as Kva,Kve = linalg.eigh(K) and cached.
   ...
   def LL(self,h,X=None,stack=True,REML=False):
    ...
    If X==None, then the default X0t will be used.  
    If X is set and stack=True, then X0t will be matrix concatenated with the input X.  
    If stack is false, then X is used in place of X0t in the LL calculation.
